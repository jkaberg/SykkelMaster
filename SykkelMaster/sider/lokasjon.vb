Public Class lokasjon
    ...
    Private Sub oppdaterGridView(Optional ByVal sok As String = Nothing)
        ...
        If Not sok = Nothing Then
            sql = "SELECT * FROM lokasjon WHERE fornavn LIKE '%" & sok & "%' OR telefon LIKE '%" & sok & "%'"
        Else
            sql = "SELECT * FROM lokasjon"
        End If
        ...
        With Me.Oppdaterlokasjon
            ...
            .Columns("Loksajon").HeaderText = "Lokasjon"
    ...
    Private Sub btnAddlocation_Click(sender As Object, e As EventArgs) Handles btnAddlocation.Click
        Dim payload As New DataTable
        Dim sql As String
        sql = "INSERT INTO lokasjon(navn, telefon, adresse, post_nr) VALUES ('" & txtLokasjon.Text & "', " & txtTelefon.Text & ", '" & txtAdresse.Text & "', '" & txtpostnr.Text & "')"
        payload = db.query(sql)
        Oppdaterlokasjon.DataSource = payload
        oppdaterGridView()
    End Sub
